...
function progress_bar_update() {
    fetch('/api/level/id')
        ...
        .then(level_ID => {
            let levels_num = level_ID.length
             levels_stup = 1 / levels_num;
             level_y +=levels_stup
            stageline.animate(level_y)
            console.log("progress its add by " + level_y);
...
const setSubmit = () => {
    ...
    if (document.getElementById('submitText').textContent === 'Finish') {
        showLoader(true);
        setInterval(()=>{ window.location.href = 'CodeNest.html'}, 2000);
        addRoby();
        addXP();
    }
